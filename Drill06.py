...
def handle_events():
    ...
    for event in events:
        ...
        elif event.type == SDL_MOUSEBUTTONUP and event.button == SDL_BUTTON_LEFT: # 마우스 누르기 효과
...
def set_new_target_arrow():
    ...
    if points: # points 안에 남아있는 점이 있으면
        sx, sy = cx, cy  # 시작점
        # hx, hy = 50,50
        hx, hy = points[0]  # 끝점
        t = 0.0
        ...
        action = 3 if action == 1 else 2 #이전에 소년이 오른쪽으로 이동중이었을때 정지 상태
        frame = 0
        target_exists = False
...
def update_world():
    ...
    if target_exists:
        if t <= 1.0:
            ...
            del points[0] # 목표지점에 왔기에 필요없는 좌표를 삭제
            set_new_target_arrow()

    elif points:  # 목표 지점이 없는 상황에서 새로운 목표지점이 생기면?
        set_new_target_arrow()
